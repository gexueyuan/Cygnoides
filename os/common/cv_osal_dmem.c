...
int  OS_Create_Memory_Pool(DM_PCB *pool_ptr, CHAR *name, 
                        ...
                        UNSIGNED min_allocation)
{
	...
	if ((start_address == NULL)||(start_address%4 != 0)) return OS_MEM_INVALID_ADDR;
...
int OS_DelMemPool(DM_PCB *pool_ptr)
{
...
int OS_GetMem(DM_PCB *pool_ptr, void **return_pointer, 
                                        UNSIGNED size)
{
...
void *osal_malloc(uint32_t size)
{
	...
	return (err == OSAL_STATUS_SUCCESS ? pointer : NULL);
}
