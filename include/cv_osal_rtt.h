 ...
 * [Task]
*/
static __inline osal_task_t *osal_task_create(
      const char* name,
      ...
      uint32_t prio)
{
    ...
    task = rt_thread_create(name, entry, param, stk_size, prio, 0xFFFFFFF0); 
    if (task) {
 ...
 * [Util]
*/
static __inline void osal_delay(uint32_t ticks)
{
...
static __inline void osal_task_schedule(void) 
{
    rt_schedule();
}
