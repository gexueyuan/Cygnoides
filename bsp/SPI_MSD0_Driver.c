...
#define INSERT_CHECK_ASK_NUM  50
/* Private variables ---------------------------------------------------------*/
...
void MSD0_SPI_Configuration(void)
{
    ...
    /* Enable the TF card ckl */
    RCC_APB2PeriphClockCmd(SD_CARD_SPI_CLK, ENABLE);

    /* Enable SCK, MOSI and MISO GPIO clocks */
    RCC_AHB1PeriphClockCmd(SD_CARD_SPI_SCK_GPIO_CLK | SD_CARD_SPI_MISO_GPIO_CLK | SD_CARD_SPI_MOSI_GPIO_CLK, ENABLE);
    /* Enable CS GPIO clocks */
    RCC_AHB1PeriphClockCmd(SD_CARD_SPI_CS_GPIO_CLK,ENABLE);

    GPIO_PinAFConfig(SD_CARD_SPI_SCK_GPIO_PORT, SD_CARD_SPI_SCK_SOURCE, SD_CARD_SPI_SCK_AF);
    GPIO_PinAFConfig(SD_CARD_SPI_MISO_GPIO_PORT, SD_CARD_SPI_MISO_SOURCE, SD_CARD_SPI_MISO_AF);
    GPIO_PinAFConfig(SD_CARD_SPI_MOSI_GPIO_PORT, SD_CARD_SPI_MOSI_SOURCE, SD_CARD_SPI_MOSI_AF);
    ...
    GPIO_InitStructure.GPIO_Pin =  SD_CARD_SPI_MOSI_PIN;
    GPIO_Init(SD_CARD_SPI_MOSI_GPIO_PORT, &GPIO_InitStructure);

    /* SPI MISO pin configuration */
    //GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN; //wanglei add
    GPIO_InitStructure.GPIO_Pin = SD_CARD_SPI_MISO_PIN;
    GPIO_Init(SD_CARD_SPI_MISO_GPIO_PORT, &GPIO_InitStructure);

    /* SPI SCK pin configuration */
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP; 
    GPIO_InitStructure.GPIO_Pin = SD_CARD_SPI_SCK_PIN;
    GPIO_Init(SD_CARD_SPI_SCK_GPIO_PORT, &GPIO_InitStructure);
    ...
    GPIO_InitStructure.GPIO_Pin = SD_CARD_SPI_CS_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_DOWN; 
    GPIO_Init(SD_CARD_SPI_CS_GPIO_PORT, &GPIO_InitStructure);
...
int MSD0_card_insert_check(void)
{
    ...
    for(retry=0; retry<INSERT_CHECK_ASK_NUM; retry++)
    {
...
int MSD0_Init(void)
{
	...
	/*for(retry=0; retry<0x000FFF; retry++)
	{
		MSD0_card_power_on();
	}*/
