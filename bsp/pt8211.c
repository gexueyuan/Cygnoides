...
static void I2S_Mode_Config(uint16_t _usStandard, uint16_t _usWordLen, uint16_t _usAudioFreq, uint16_t _usMode)
{
	...
	{
		I2S_InitStructure.I2S_Mode = I2S_Mode_MasterTx;			/* ????I2S1¡è¡Á¡Â?¡ê¨º? */
		I2S_InitStructure.I2S_Standard = _usStandard;			/* ?¨®?¨²¡À¨º¡Á? */
		I2S_InitStructure.I2S_DataFormat = _usWordLen;			/* ¨ºy?Y??¨º?¡ê?16bit */
		I2S_InitStructure.I2S_MCLKOutput = I2S_MCLKOutput_Disable;	/* ?¡Â¨º¡À?¨®?¡ê¨º? */
		I2S_InitStructure.I2S_AudioFreq = _usAudioFreq;			/* ¨°??¦Ì2¨¦?¨´?¦Ì?¨º */
		I2S_InitStructure.I2S_CPOL = I2S_CPOL_Low;  			
		I2S_Init(I2S, &I2S_InitStructure);
	}
	else if (_usMode == I2S_Mode_MasterRx)
	{
		I2S_InitStructure.I2S_Mode = I2S_Mode_MasterRx;			/* ????I2S1¡è¡Á¡Â?¡ê¨º? */
		I2S_InitStructure.I2S_Standard = _usStandard;			/* ?¨®?¨²¡À¨º¡Á? */
		I2S_InitStructure.I2S_DataFormat = _usWordLen;			/* ¨ºy?Y??¨º?¡ê?16bit */
		I2S_InitStructure.I2S_MCLKOutput = I2S_MCLKOutput_Disable;	/* ?¡Â¨º¡À?¨®?¡ê¨º? */
		I2S_InitStructure.I2S_AudioFreq = _usAudioFreq;			/* ¨°??¦Ì2¨¦?¨´?¦Ì?¨º */
...
void I2S_StartPlay(uint16_t _usStandard, uint16_t _usWordLen, uint16_t _usAudioFreq)
{
	...
	SPI_I2S_ITConfig(I2S, SPI_I2S_IT_TXE, ENABLE);		/* ¨º1?¨¹¡¤¡é?¨ª?D?? */
}


/*
*********************************************************************************************************
*	o¡¥ ¨ºy ??: I2S_CODEC_DataTransfer
*	1|?¨¹?¦Ì?¡Â: I2S¨ºy?Y¡ä?¨º?o¡¥¨ºy, ¡À?SPI2 (I2S)?D??¡¤t??3¨¬D¨°¦Ì¡Â¨®??¡ê??¨°?o¨ª¡¤?¨°??¨´?¨²¡ä?o¡¥¨ºy¡ä|¨¤¨ª?¡ê
*	D?    2?¡êo?T
*	¡¤¦Ì ?? ?¦Ì: ?T
*********************************************************************************************************
*/
