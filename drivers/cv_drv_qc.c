...
{   
    ...
    if(p_qc->qc_periph.usb.state) {
        OSAL_MODULE_DBGPRT(MODULE_NAME,OSAL_DEBUG_INFO,">>usb init test OK.\n");
    }else {
        OSAL_MODULE_DBGPRT(MODULE_NAME,OSAL_DEBUG_INFO,">>usb init test error:%d\n",p_qc->qc_periph.gsensor.error);
        qc_led_type = (qc_led_type > QC_LED_011)?QC_LED_011:qc_led_type;
    }
    
    if(TRUE == log_store.sd_state) {
        OSAL_MODULE_DBGPRT(MODULE_NAME,OSAL_DEBUG_INFO,">>TF card test OK.\n");
    }else {
        OSAL_MODULE_DBGPRT(MODULE_NAME,OSAL_DEBUG_INFO,">>TF card test error:0\n");
    }
}
