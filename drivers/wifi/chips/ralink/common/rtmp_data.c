    ...
    IN ULONG         RxBufferLength)
{
    ...
    {/* Parse the the element of V2V */
        ...
        while (PayloadLen > 0) {
            ...
            ElementLen = *pPayload++;
            if (ElementID == 0xDD) {
                /* receive rsu frame: 80211header|Time/LLC|Reserved|ID|len|Dsmp+rcp| */
                //osal_printf("."); /* Indicate RX is in process, for debug only */
                /* move LLC header */ 
                pPayload -= 8;
            ...
            if (ElementID == 0xAA) {
                /* receive rsu frame: 80211header|Timestrmp|Reserved|LLC+Dsmp+rcp| */
                pPayload -= 2;
#if 0
                osal_printf("recv rsu frame len=%d\r\n", PayloadLen);
                osal_dbg_dump_data(pPayload, (UINT32)PayloadLen);
#endif
                wnet_recv(&rxinfo, pPayload, (UINT32)PayloadLen);
                break;
            }
            pPayload += ElementLen;
