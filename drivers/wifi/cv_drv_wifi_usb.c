...
wifi_usb_init_state_t wifi_usb_init;
...
static void _dev_probe_entry(void *parameter)
{
    ...
    */
    ...
    /* usb init ok */
    wifi_usb_init.check = TRUE;
    wifi_usb_init.state = TRUE;
...
int8_t usb_init_check(void) 
{
    if (TRUE == wifi_usb_init.check) {
        return wifi_usb_init.state;
    }
    else {
        return -1;
    }
...
static void usbh_thread_entry(void *parameter)
{
    ...
    osal_timer_start(wifi_usb_init.usb_check_timer);
    while (1) {
        /* Host Task handler. */
...
void usb_init_check_callback(void* parameter)
{
    wifi_usb_init.check = TRUE;
    if (TRUE != wifi_usb_init.state) {
        DBGPRINT(WHED_DEBUG_ERROR,"usb init error!\n");
        osal_task_del(wifi_usb_init.usb_h);
        wifi_usb_init.usb_h = NULL;
        if(p_cms_envar->sys.queue_sys_mng) {
            sys_add_event_queue(&p_cms_envar->sys, SYS_MSG_INITED, 0, 0, 0);
        }
        osal_task_schedule();
...
void usb_init(void)
{
    ...
    memset(&wifi_usb_init,0,sizeof(wifi_usb_init_state_t));
    ...
    wifi_usb_init.usb_check_timer = osal_timer_create("usb_c", usb_init_check_callback,(void *)NULL,WIFI_USB_INIT_TIME_TICK,RT_TIMER_FLAG_ONE_SHOT);
    osal_assert(wifi_usb_init.usb_check_timer != NULL);

    /* Finally start USB host process thread */
    wifi_usb_init.usb_h = osal_task_create("usbh", usbh_thread_entry, NULL, RT_USBH_THREAD_STACK_SIZE, \
                            RT_USBH_THREAD_PRIORITY);
    osal_assert(wifi_usb_init.usb_h != NULL);
}
